...
QSize ctkPathLineEditPrivate::recomputeSizeHint(QSize& sh)const
{
    ...
    if (!sh.isValid())
    {
        ...
        if (&sh == &this->SizeHint || this->MinimumContentsLength == 0)
        {
            ...
            case ctkPathLineEdit::AdjustToContentsOnFirstShow:
                ...
                {
                    textWidth = 7 * this->LineEdit->fontMetrics().horizontalAdvance(QLatin1Char('x'));
                }
                else
                {
        ...
        {
            textWidth = qMax(textWidth, this->MinimumContentsLength * this->LineEdit->fontMetrics().horizontalAdvance(QLatin1Char('X')));
        }
    ...
    return sh.expandedTo(QSize(10,10));
}
...
QString extensionToRegExp(const QString& extension)
{
    ...
    auto match =  extensionExtractor.match(extension);
    int pos = match.lastCapturedIndex();
    if (pos < 0)
    {
        return QString();
    }
...
QRegularExpression nameFiltersToRegExp(const QStringList& nameFilters)
{
    QString pattern;
    foreach(const QString& nameFilter, nameFilters)
    {
...
void ctkPathLineEditPrivate::updateFilter()
{
    ...
    QRegularExpressionValidator* validator = new QRegularExpressionValidator(
                nameFiltersToRegExp(this->NameFilters), q);
