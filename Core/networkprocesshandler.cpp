...
CheckoutHttpClient::CheckoutHttpClient(QString host, quint16 port)
    : awaiting(false), icpus(0), collapse(true), run(true) {
  iurl.setScheme("http");
  iurl.setHost(host);
  iurl.setPort(port);
...
void CheckoutHttpClient::send(QString path, QString query, QJsonArray ob,
                              bool keepalive) {
  // auto body = QCborValue::fromJsonValue(ob).toCbor();
  //     qDebug() << "Generating cbor array" << ob.size() <<
  //     QCborArray::fromJsonArray(ob).size();
...
void CheckoutHttpClient::send(QString path, QString query, QByteArray ob,
                              bool keepalive) {
...
void CheckoutHttpClient::sendQueue() {
  ...
  while (run) {
    ...
    if (collapse) {
      ...
      for (int i = 0; i < reqs.size() && collapsed.size() < 500;
           ++i) // do not merge more than 500 reqs
    ...
    if (collapsed.size() > 0) {
      ...
      for (QList<int>::reverse_iterator it = collapsed.rbegin(),
                                        e = collapsed.rend();
           it != e; ++it)
        reqs.removeAt(*it);
    ...
    qDebug() << QDateTime::currentDateTime().toString() << "Sending Queued"
             << url.url() << ob.size();

    iclient.push_back(new QHttpClient());
    auto icli = iclient.back();

    QObject::connect(icli, &QHttpClient::disconnected,
                     [this]() { this->finalize(); });

    icli->request(
        qhttp::EHTTP_POST, req.url,
...
void CheckoutHttpClient::onIncomingData(const QByteArray &data) {
  ...
  if (ob.contains("Processes")) {
    NetworkProcessHandler::handler().setProcesses(ob["Processes"].toArray(),
                                                  this);
    return;
  }
  ...
  if (ob.contains("ArrayCoreProcess") && ob.contains("ArrayRunProcess")) {
...
NetworkProcessHandler::NetworkProcessHandler()
    : session(nullptr), _waiting_Update(false), last_serv_pos(0) {
...
void NetworkProcessHandler::setNoProxyMode() { srv = QString(); }

void NetworkProcessHandler::addProxyPort(uint16_t port) {
  srv = QString("%1.%2").arg(srv).arg(port);
}

void NetworkProcessHandler::setServerAddress(QString srv_) { srv = srv_; }

QString NetworkProcessHandler::getServer() { return srv; }

void NetworkProcessHandler::setProcesses(QJsonArray ar,
                                         CheckoutHttpClient *cl) {
...
void NetworkProcessHandler::startProcess(CheckoutHttpClient *h, QString process,
                                         QJsonArray ob) {
  h->send(QString("/Start/%1").arg(process), QString(), ob);
}

// Modified Core/Server Exchange to better handle heavy interaction (here high
// network load) Large image sets are at risk for the handling of data this may
// lock the user interface with heavy processing
QJsonArray ProcessMessageStatusFunctor(CheckoutHost *h, QList<QString> hash,
                                       NetworkProcessHandler *owner) {
  Q_UNUSED(h);
  Q_UNUSED(hash);
  Q_UNUSED(owner);
...
// The Process message status can be a bit long, though the current network
// approach is not at it's best handling the message at threaded approach would
// be better of handling this network information
void NetworkProcessHandler::getProcessMessageStatus(QString process,
                                                    QList<QString> hash) {
  Q_UNUSED(process);
  Q_UNUSED(hash);
}
...
QJsonArray NetworkProcessHandler::filterObject(QString hash, QJsonObject ds,
  ...
  QString plate = ds["XP"].toString().replace("\\", "/"),
          commit = ds["CommitName"].toString();

  if (plate.contains("/"))
    plate = plate.split("/").last();
  ...
  if (store.outfile.isEmpty() && !commit.isEmpty()) {
    ...
    dbP = QString("/mnt/shares/L");
#endif

    // FIXME:  Cloud solution adjustements needed here !

    dbP.replace("\\", "/").replace("//", "/");
    QString folder = QString("%1/PROJECTS/%2/Checkout_Results/%3")
                         .arg(dbP, ds["Project"].toString(), commit);

    QDir d;
    if (!d.exists(
            folder)) // Ensure the folder exists for writing the feather file
      d.mkpath(folder);

    store.outfile = QString("%1/%4%5.fth")
                        .arg(folder, plate, srv)
                        .replace("\\", "/")
                        .replace("//", "/");
    store.plate = plate;
    ...
    if (store.fresh) {
      if (QFile::exists(store.outfile)) {
        int tgt = 0;
        QString path = QString("%1/%4%5%6.fth")
                           .arg(folder, plate, srv)
                           .arg(tgt)
                           .replace("\\", "/")
                           .replace("//", "/");
  ...
  std::map<QString, QString> tr = {
      {"FieldId", "fieldId"}, {"zPos", "sliceId"}, {"TimePos", "timepoint"},
      {"Channel", "channel"}, {"Pos", "Well"},     {"WellTags", "tags"}};
  ...
  if (ds.contains("Data")) {
    auto arr =
        ds["Data"]
            .toArray(); // Theorically at this level only one Data in this table
    for (auto itd : arr) {
      ...
      if (obj["Data"].toString() != "Image results") {
          ...
          for (auto &s : txt)
            if (met.contains(s)) {
              ...
              if (commit.isEmpty())
                ...
                dfKey += (v.isString() ? v.toString()
                                       : QString("%1").arg((int)v.toDouble())) +
                         ";";
            } else {
              if (!commit.isEmpty())
                dfKey += "-1;";
        ...
        if (!commit.isEmpty()) {
          ...
          if (obj["Data"].isDouble())
            ...
            if (t.contains(';')) {
              ...
              for (int i = 0; i < items; ++i)
                store.arrFl[QString("%1#%2").arg(name).arg(
                    i, 4, 10, QLatin1Char('0'))][dfKey] = t2[i].toDouble();
            } else
          ...
          if (items == 0)
            ...
            for (int i = 0; i < items; ++i)
              store.fuseT[QString("%1#%2").arg(name).arg(
                  i, 4, 10, QLatin1Char('0'))] = obj["Aggregation"].toString();
...
void NetworkProcessHandler::storeObject(QString commit, bool finished) {
  ...
  for (auto it = plateData.begin(), end = plateData.end(); it != end; ++it) {
    if (it.key().endsWith(commit)) {
      ...
      auto res =
          QtConcurrent::run(&NetworkProcessHandler::storeData, this, name, tt);
...
mdcli &NetworkProcessHandler::getSession() {
  ...
  if (session == nullptr) {
    QSettings set;
    auto srv = QString("tcp://%1:%2")
                   .arg(set.value("ZMQServer", "localhost").toString())
    ...
    QString hash =
        QCryptographicHash::hash(indata, QCryptographicHash::Md5).toHex();
...
int NetworkProcessHandler::DoneJobCount() { return jobcount; }

int NetworkProcessHandler::OngoingJobCount() { return ongoingjob; }

int NetworkProcessHandler::FinishedJobCount() { return finishedjobs; }
...
void exportBinary(QJsonObject &ds, QJsonObject &par,
                  QCborMap &ob) // We'd like to serialize this one
{
  ...
  if (!commit.isEmpty() && par.contains(QString("SavePath")) &&
      !par.value("SavePath").toString().isEmpty()) {
    auto meta = par["Meta"].toArray().first().toObject();
    QString pos = meta["Pos"].toString();

    int t = meta["TimePos"].toInt(), f = meta["FieldId"].toInt(),
        z = meta["zPos"].toInt(), ch = meta["Channel"].toInt();
    ...
    QString tofile =
        QString("%1/%2/%3_%5_%4.fth")
            .arg(par.value("SavePath").toString(), commit, plate, ttag, pos);
    //        qDebug() << "Saving Generated Meta to" << tofile;
    {
      ...
      int r = ob.value(QCborValue("Rows")).toInteger(),
          c = ob.value(QCborValue("Cols")).toInteger();
      ...
      for (int c = 0; c < feat.cols; ++c)
        if (c < cols.size())
          fields.push_back(
              arrow::field(cols[c].toStdString(), arrow::float32()));
        else
          fields.push_back(arrow::field((QString("%1").arg(c)).toStdString(),
                                        arrow::float32()));
      ...
      //        options.codec =
      //        arrow::util::Codec::Create(arrow::Compression::LZ4).ValueOrDie();
      //        //std::make_shared<arrow::util::Codec>(codec);

      auto r2 =
          arrow::ipc::MakeFileWriter(output.get(), table->schema(), options);
...
void NetworkProcessHandler::lockDataSaver() { plate_data_locker.lock(); }

void NetworkProcessHandler::unlockDataSaver() { plate_data_locker.unlock(); }
          ...
          if (pay.contains("DataHash")) {
            ...
            try {
              while (pos < buf.size()) {
                ...
                ar.append(QByteArray(reinterpret_cast<const char *>(&buf[pos]),
                                     (int)end));
                pos += end;
              }
            } catch (...) {
            ...
            qDebug() << "Got binary data: " << dhash << ds["XP"].toString()
                     << ds["CommitName"].toString()
                     << obj.value("Tag").toString() << buf.size() << ar.size()
                     << pos;
...
void NetworkProcessHandler::removeHash(QStringList hashes) {
  ...
  for (auto &hash : hashes) {
    ...
    if (runningProcs.remove(hash) == 0)
      qDebug() << "hash " << hash << "not found"
               << (olds.contains(hash)
                       ? QString("but was already seen %1").arg(olds[hash])
                       : QString(""));
  }
...

  {
    ...
    for (auto &h : df.arrStr.keys()) {
      ...
      if (df.fuseT.contains(h))
        meta = arrow::KeyValueMetadata::Make({"Aggregation"},
                                             {df.fuseT[h].toStdString()});
    ...
    qDebug() << "Storing DataFile to:" << target << " Rows "
             << dat.front()->length();
...
void NetworkProcessHandler::displayError(
...
void NetworkProcessHandler::watcherProcessStatusFinished() {
  QFutureWatcher<QJsonArray> *watcher =
      dynamic_cast<QFutureWatcher<QJsonArray> *>(sender());
